module Mojaik
  class WalletAbstract < Peatio::Wallet::Abstract
    ...
    def create_address!(options = {})
      puts "37 option => #{options}"
      uid = options.fetch(:uid) + '_' + (Time.now.getutc.to_i).to_s
      puts "42 uid => #{uid}"
      secret = [*('a'..'z'),*('A'..'Z'),*('0'..'9')].shuffle[0,16].join
      secret.yield_self do |password|
        { address: normalize_address(client.json_rpc(:personal_newAccount, [uid, password])),
          secret:  password }
      end
    ...
    def create_eth_transaction!(transaction, options = {})
      ...
      transaction.options = options
      puts "150 transaction ==> #{transaction}"
      transaction
    end
    ...
    def create_erc20_transaction!(transaction, options = {})
      ...
      txid = client.json_rpc(:personal_sendTransaction_erc20,
                [{
                    from:     normalize_address(@wallet.fetch(:address)),
                    to:       normalize_address(transaction.to_address),
                    # data:     data,
                    ...
                    gasPrice: '0x' + options.fetch(:gas_price).to_i.to_s(16),
                    contractAddress: options.fetch(contract_address_option),
                    value: '0x' + amount.to_s(16)
                  }.compact, @wallet.fetch(:secret)])
