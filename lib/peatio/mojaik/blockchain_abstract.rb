module Mojaik
  class BlockchainAbstract < Peatio::Blockchain::Abstract
    ...
    def configure(settings = {})
      ...
      puts "34 configure => #{@settings[:currencies]}"
      @settings[:currencies]&.each do |c|
        puts "36 c => #{c}"
        puts "37 native_currency_id => #{native_currency_id}"
    ...
    def fetch_block!(block_number)
      ...
      puts "49 block_json ===> #{block_json}"
      puts "50 block_json['transactions'] ===> #{block_json['transactions']}"
      ...
      block_json.fetch('transactions').each_with_object([]) do |tx, block_arr|
        puts "60 tx.fetch('input').hex ==> #{tx.fetch('input').hex}"
        if tx.fetch('input').hex <= 0
          next if invalid_eth_transaction?(tx)
        else
          process_tx = false
          puts "65 process_tx ==> #{process_tx}"
          ...
          puts "93 process_tx before ===> #{process_tx}"
          next unless process_tx
          puts "95 process_tx after ===> #{process_tx}"
    ...
    def client
      @client ||= Mojaik::Client.new(settings_fetch(:server))
    end
    ...
    def build_eth_transactions(block_txn)
      puts "197 block_txn.fetch('value').hex ===> #{block_txn.fetch('value').hex}"
      puts "@eth ===> #{@eth}"
      [
        {
    ...
    def convert_from_base_unit(value, currency)
      puts "283 value ===> #{value}"
      puts "284 currency ===> #{currency}"
      puts "285 value/currency ===> #{value.to_d / currency.fetch(:base_factor).to_d}"
